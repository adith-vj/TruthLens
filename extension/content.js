...
class TranscriptProvider {
    ...
    async acquire(capability) {
        ...
        if (capability.platform === 'youtube') {
            ...
            try {
                const response = await fetch('http://127.0.0.1:8000/api/video/transcript', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (!response.ok) {
                    ...
                    try {
                        ...
                        errMsg = errorData.detail || errorData.message || response.statusText;
                    } catch (e) {
                        errMsg = response.statusText;
                    }
                    throw new Error(`Failed to retrieve transcript: ${errMsg}`);
                }
                ...
                this.cachedTranscript = {
                    ...
                    segments: data.segments // Already normalized by backend
                };

                return this.cachedTranscript;
            } catch (err) {
                console.error("TruthLens Backend Fetch Error:", err);
                throw err;
            }
        } else {
